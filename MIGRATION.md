...
Сценарий оплаты через `Sberpay` теперь происходит не в мобильном приложения, а через встроенное Sber SDK. Для его работы потребуется уникальная URL-scheme, которую необходимо запросить у поддержки b2b_support@yoomoney.ru (см. подробнее в README)

Чтобы поддержать новый сценарий необходимо внести изменения в вашем файле `Info.plist`:
